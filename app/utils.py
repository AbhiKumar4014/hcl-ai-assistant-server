    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for site_name, url_list in hcl_urls.items():
            ...
            future = executor.submit(
                extract_text_from_url, url_list[0]
            )  # Assuming only first URL is needed
...
def extract_channel_videos(
    channel_id: str = "UC07b9GB8a-4c6-T6pd2bbBQ", limit: int = None
):
    ...
    for video in videos:
        ...
        description = (
            video.get("descriptionSnippet", {}).get("runs", [{}])[0].get("text")
...
def create_doc_embeddings(docs, embeddings):
    ...
    for key in docs:
        for doc in docs[key]:
            ...
            document = Document(
                page_content=combined_text, metadata={"title": title, "url": url}
            )
...
def validate_document_url(url: str) -> bool:
    url_pattern = re.compile(r"^(https?://)?(www\.)?([\w-]+)\.[a-z]{2,6}(/\S*)?$")
